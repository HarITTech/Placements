...
function Home() {
  ...
  useEffect(() => {
    if (user?.profile) {
      setFormData({
        ...
        subcollege: user.profile.subcollege || "",
    ...
    if (isEditing) {
      ...
      const updates = {
        profile: {
          ...
          subcollege: formData.subcollege,
  ...
  const handlePreviewPDF = () => {
    try {
      ...
      // College Name (wrapped if too long)
      const collegeName =
        formData.collegename ||
        "Tulsiramji Gaikwad Patil College of Engineering and Technology, Nagpur";
      const collegeLines = doc.splitTextToSize(collegeName, 140); // Wrap text
      doc.text(collegeLines, 50, 30); // Display wrapped text

      // Subcollege (Smaller font, positioned correctly)
      doc.setFontSize(9);
      doc.setFont("helvetica", "normal");
      if (formData.subcollege) {
        doc.text(formData.subcollege, 50, 35); // Adjusted position
      }

      // College Address
      doc.setFontSize(8);
      doc.text(formData.collegeaddress || "Nagpur", 50, 40); // Adjusted position
      ...
      doc.setFontSize(6);
      doc.textWithLink("Visit TNP Portal by HarIT Tech Solution", 50, 44, {
        url: collegeWebsite,
      });

      // Reset text color to default (black) after the link
      ...
      {loading ? (
        ...
        <div className="w-full mx-auto border-[2px] border-gray-600 bg-opacity-40 bg-white backdrop-blur-md rounded-lg shadow-lg p-6">
            ...
            <div className="flex items-center gap-6">
              ...
              <div>
                ...
                <div className="">
                  ...
                  <p className="text-md mt-1 font-bold text-green-600">
                    {formData.collegename}{" "}
                  </p>
          ...
          <div className="mt-8 grid gap-6 md:grid-cols-2">
            ...
            <div>
              <label className="block text-md font-semibold text-gray-700">
                Sub College <span className="text-red-500">*</span>
              </label>
              <select
                id="subcollege"
